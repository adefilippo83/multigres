...
const (
	// Special startup messages (no type byte, identified by protocol version)
	MsgTypeStartup       = 0    // Regular startup message
	MsgTypeCancelRequest = 0    // Cancel request (identified by CancelRequestCode)
	MsgTypeSSLRequest    = 0    // SSL request (identified by SSLRequestCode)
	MsgTypePassword      = 'p'
	MsgTypeQuery         = 'Q'
	MsgTypeParse         = 'P'
	MsgTypeBind          = 'B'
...
const (
	TypeOIDInt4 = 23  // 32-bit integer
	TypeOIDText = 25  // Variable-length text
	TypeOIDInt8 = 20  // 64-bit integer
	TypeOIDBool = 16  // Boolean
)

// CreateSimpleRowDescription creates a row description for a single column
// This is a utility function for simple query responses
func CreateSimpleRowDescription(columnName string, typeOID uint32) []byte {
	...
	switch typeOID {
	case TypeOIDInt4:
		data = append(data, 0, 4) // 4 bytes for int4
	case TypeOIDInt8:
		data = append(data, 0, 8) // 8 bytes for int8
	case TypeOIDBool:
		data = append(data, 0, 1) // 1 byte for bool
	default:
		data = append(data, 0xFF, 0xFF) // -1 for variable length
	}
...
func CreateSimpleDataRow(value string) []byte {
	data := make([]byte, 0, 256)
