...
func (s *Server) handleConnectionLifecycle(conn *Connection) error {
	// Read startup message (may be SSL request)
	...
	if startup.IsSSLRequest {
		if s.config.TLSConfig != nil && s.config.TLSConfig.CertFile != "" {
			// Accept SSL
			if err := conn.Writer.WriteSSLResponse(true); err != nil {
				return err
			}
			// TODO: Upgrade connection to TLS
			s.logger.Info("SSL requested and accepted (TLS upgrade pending implementation)")
		} else {
			// Reject SSL
			if err := conn.Writer.WriteSSLResponse(false); err != nil {
				return err
			}
			s.logger.Debug("SSL requested but rejected (no TLS config)")
		}
		
		// Read the actual startup message after SSL negotiation
		startup, err = conn.Reader.ReadStartupMessage()
		if err != nil {
			return fmt.Errorf("failed to read startup message after SSL negotiation: %w", err)
		}
	}
...
func (s *Server) handleSSLNegotiation(conn *Connection) error {
	// The startup message reader will check for SSL request
	// We don't need to peek here - let ReadStartupMessage handle it
	// SSL requests are identified by the SSLRequestCode in the startup message
	return nil
}
